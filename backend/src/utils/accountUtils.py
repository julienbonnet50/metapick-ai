...
def get_cost_and_score_by_account(player_tag, data, api_key, base_url):
    accountStats = battlesUtils.get_account_brawlers(player_tag=player_tag, API_KEY=api_key, BASE_URL=base_url)
    ...
    for accountBrawler in accountBrawlers:
        ...
        res.append({"name": name, "score": sorted_scores.get(name), "total_power_points": brawler_upgrade.total_power_points, "total_coins": brawler_upgrade.total_coins})
    return res