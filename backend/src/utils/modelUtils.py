    ...
    def __init__(self, df, brawler_to_idx, map_to_idx, pad_idx=0):
        ...
        for idx, row in df.iterrows():
            ...
            for i in range(3):
                ...
                friends += [pad_idx] * (
                    2 - len(friends)
                )  # Pad friends to 2 (since one is hidden)
...
class EnhancedBrawlerPredictionModel(nn.Module):
    def __init__(
        ...
        self.brawler_embedding = nn.Embedding(
            num_brawlers, emb_dim, padding_idx=pad_idx
        )
    ...
    def forward_old(self, friends, enemies, map_idx):
        ...
        map_attention = torch.sigmoid(self.map_brawler_attention(emb_map)).unsqueeze(
            1
        )  # [batch, 1, emb_dim]

        # Apply attention mechanism for better feature interaction
        all_brawlers = torch.cat(
            [emb_friends, emb_enemies], dim=1
        )  # [batch, num_friends+num_enemies, emb_dim]
        attended_brawlers, _ = self.attention(all_brawlers, all_brawlers, all_brawlers)

        # Flatten and concatenate
        flat_friends = emb_friends.reshape(
            batch_size, -1
        )  # [batch, num_friends*emb_dim]
        flat_enemies = emb_enemies.reshape(
            batch_size, -1
        )  # [batch, num_enemies*emb_dim]

        # Concatenate all features
        x = torch.cat(
            [flat_friends, flat_enemies, emb_map], dim=1
        )  # [batch, input_dim]
    ...
    def forward(self, friends, enemies, map_idx):
        ...
        enemy_positions = (
            enemy_positions.unsqueeze(0).unsqueeze(2).expand(batch_size, -1, 1)
        )
        ...
        enemy_attention_out, _ = self.enemy_attention(
            augmented_enemies, augmented_enemies, augmented_enemies
        )

        # Separate attention for friends
        friend_attention_out, _ = self.friend_attention(
            emb_friends, emb_friends, emb_friends
        )

        # Cross-attention between friends and enemies
        # This helps the model understand how friends match up against the enemy team
        cross_attention_out, _ = self.cross_attention(
            friend_attention_out, enemy_attention_out, enemy_attention_out
        )

        # Flatten and concatenate with appropriate weighting
        flat_friends = cross_attention_out.reshape(
            batch_size, -1
        )  # [batch, num_friends*emb_dim]
        flat_enemies = enemy_attention_out.reshape(
            batch_size, -1
        )  # [batch, num_enemies*emb_dim]
        ...
        for i in range(batch_size):
            ...
            for j in range(self.num_enemies):
                ...
                if enemy_idx != self.pad_idx:
                    ...
                    counter_influence += (
                        self.counter_matrix[enemy_idx] * position_importance
                    )
            ...
            if valid_enemies > 0:
                ...
                counter_scaling = (
                    1.0  # This parameter controls how much additional enemies matter
                )
