...
class NeuralNetworkService:
    def __init__(self, appConfig=None, data_path="", model_path="", device=None, version="35", autoLoadEnable=True):
        ...
        self.counter_matrix_path = self.model_path.replace('nn_model_all.pth', 'counter_matrix.pkl')
        self.plot_path = self.model_path.replace('nn_model_all.pth', 'training_loss_plot.png')
    ...
    def load_counter_matrix(self):
        ...
        if os.path.exists(self.counter_matrix_path):
            ...
            try:
                ...
                self.counter_matrix = torch.tensor(self.counter_matrix, device=self.device)
    ...
    def train_model2(self, num_epochs=10, batch_size=64, num_friends=3, num_enemies=3, val_split=0.1, patience=5):
        # Ensure we have mappings
        if self.brawler_to_idx is None or self.map_to_idx is None:
            raise ValueError("Mappings not initialized. Please load or build mappings before training.")
    ...
    def predict_best_brawler(self, friends, enemies, map_name, excluded=[], nbBrawlers=10, available_brawlers=[]):
        ...
        with torch.no_grad():
            ...
            counter_strength = 2.0  # How strongly to boost counter picks (adjust as needed)
            ...
            for enemy in enemies:
                if enemy in self.brawler_to_idx:
                    ...
                    for brawler_idx in range(len(self.brawler_to_idx)):
                        ...
                        counter_value = self.get_counter_strength(brawler_idx, enemy_idx)
                        
                        # Boost the score for strong counters
                        if counter_value > 0.5:  # This is a strong counter
                            counter_boost[0, brawler_idx] += counter_value * counter_strength
    ...
    def save_counter_matrix(self):
        ...
        def process_data_point(data_point):
            ...
            try:
                friends = [data_point[f'friend{i+1}'] for i in range(self.model.num_friends)]
                enemies = [data_point[f'enemy{i+1}'] for i in range(self.model.num_enemies)]
        ...
        with ThreadPoolExecutor() as executor:
            future_to_data = {executor.submit(process_data_point, data): data for data in self.dataset}
            
            with tqdm(total=len(self.dataset), desc="Processing Matchups", unit=" match") as pbar:
        ...
        with tqdm(total=len(matchup_counts), desc="Calculating Counter Strengths", unit=" pair") as pbar:
            for (friend, enemy), count in matchup_counts.items():
                if count >= 10:  # Only consider matchups with enough data
                    ...
                    counter_strength = (win_rate - 0.5) * 2  # Rescale from [0.5, 1.0] to [0, 1.0]
        ...
        try:
            ...
            for (friend, enemy), count in matchup_counts.items():
                if count >= 10:  # Only consider matchups with enough data
                    ...
                    counter_strength = max(0, counter_strength)  # Ensure non-negative
                    counter_matrix[friend, enemy] = counter_strength
                    print(f"Set counter strength for ({friend}, {enemy}): {counter_strength:.4f}")
